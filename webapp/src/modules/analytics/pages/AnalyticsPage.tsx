...
export function AnalyticsPage() {
  ...
  async function handleCreateSavedReport() {
    if (!entityKey || !savedName || savedRecipientIds.length === 0 || savingSchedule) return
    setError(null)
    setMessage(null)
  ...
  async function handleBuildReport() {
    if (!entityKey || buildingReport) return
    setError(null)
      ...
      {isHrAdmin && (
        <Card>
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Button size="sm" variant="outline" onClick={handleBuildReport} disabled={!entityKey || buildingReport}>
              {buildingReport ? 'Running…' : 'Run Report'}
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleExport('csv')} disabled={!entityKey || exportingFormat !== null}>
              {exportingFormat === 'csv' ? 'Exporting…' : 'Export CSV'}
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleExport('excel')} disabled={!entityKey || exportingFormat !== null}>
              {exportingFormat === 'excel' ? 'Exporting…' : 'Export Excel'}
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleExport('pdf')} disabled={!entityKey || exportingFormat !== null}>
        ...
        <Card>
          ...
          <ul className="mb-3 flex flex-col gap-2">
            {savedReports.map((r) => (
              <li key={r.id} className="flex items-center justify-between rounded border p-2">
                ...
                >
                  {deletingScheduleId === r.id ? 'Deleting…' : 'Delete'}
                </Button>
              </li>
            ))}
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select value={savedSchedule} onValueChange={(v) => setSavedSchedule((v as ReportSchedule) ?? 'WEEKLY')}>
            ...
            <Button size="sm" variant="outline" onClick={handleCreateSavedReport} disabled={!entityKey || savingSchedule}>
              {savingSchedule ? 'Saving…' : 'Save Schedule (uses filters above)'}
            </Button>
          </div>
