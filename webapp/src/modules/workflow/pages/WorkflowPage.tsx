...
export function WorkflowPage() {
  ...
  // Guards a single in-flight decision so a slow response can't be
  // double-fired by a second click on the same (or the opposite) button.
  ...
  async function handleCreateDefinition() {
    if (!name || !moduleName || creatingDefinition) return
    setError(null)
    setMessage(null)
            ...
            <li key={`${a.source}-${a.id}`} className="flex items-center justify-between gap-2 rounded-md border p-2">
              ...
              {a.source === 'WORKFLOW' && (
                <div className="flex gap-2">
                  ...
                  >
                    {deciding?.id === a.id && deciding.decision === 'APPROVED' ? 'Approving…' : 'Approve'}
                  </Button>
                  <Button
                    size="sm"
                    variant="destructive"
                  ...
                  >
                    {deciding?.id === a.id && deciding.decision === 'REJECTED' ? 'Rejecting…' : 'Reject'}
                  </Button>
                </div>
              )}
            </li>
      ...
      {isHrAdmin && (
        <Card>
          ...
          <p className="mb-3 text-muted-foreground">
            An extra approval chain attached to a module, on top of its normal approver — e.g.
            &quot;over a certain amount, a second person must also sign off.&quot;
          </p>
          ...
          <div className="flex flex-col gap-4 rounded-md border bg-muted/20 p-4">
                      ...
                      <div key={ruleIndex} className="flex flex-wrap items-center gap-2">
                        <Select
                          ...
                          onValueChange={(v) =>
                            updateApproverRule(stepIndex, ruleIndex, {
                              type: (v as ApproverRuleType) ?? 'MANAGER',
                        ...
                        {rule.type === 'ROLE' && (
                          <Select
                            ...
                            onValueChange={(v) => updateApproverRule(stepIndex, ruleIndex, { role: v ?? 'MANAGER' })}
                          >
                    ...
                    {step.slaHours && (
                      <div className="flex flex-col gap-1">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            updateStep(stepIndex, { escalationTargetRole: v === 'NONE' || !v ? '' : v })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          updateStep(stepIndex, { conditionOperator: (v as StepCondition['operator']) ?? 'gt' })
                        }
                      >
            ...
            <Button size="sm" disabled={creatingDefinition} onClick={handleCreateDefinition} className="self-start">
