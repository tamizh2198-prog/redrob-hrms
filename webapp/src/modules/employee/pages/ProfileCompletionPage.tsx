...
export function ProfileCompletionPage() {
  ...
  const [savingMode, setSavingMode] = useState<'continue' | 'later' | null>(null)
  ...
  async function handleSave(continueOnComplete: boolean) {
    if (saving) return
    setSaving(true)
    setSavingMode(continueOnComplete ? 'continue' : 'later')
    setError(null)
    setMessage(null)
    try {
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6 p-6">
      ...
      <div className="flex gap-2">
        <Button disabled={saving} onClick={() => handleSave(true)}>
          {savingMode === 'continue' ? 'Saving…' : isComplete ? 'Save' : 'Save & Continue'}
        </Button>
        {!isComplete && (
          <Button variant="outline" disabled={saving} onClick={() => handleSave(false)}>
            {savingMode === 'later' ? 'Saving…' : 'Save & Complete Later'}
          </Button>
        )}
      </div>
    </div>
