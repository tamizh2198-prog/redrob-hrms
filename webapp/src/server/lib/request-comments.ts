import type { PrismaClient, RequestCommentType } from "@prisma/client";
...
export async function addSuperAdminComment(
  prisma: PrismaClient,
  params: { requestType: RequestCommentType; requestId: string; authorId: string; body: string },
) {
  return prisma.superAdminRequestComment.create({ data: params });
}

export async function listSuperAdminComments(prisma: PrismaClient, requestType: RequestCommentType, requestId: string) {
  return prisma.superAdminRequestComment.findMany({ where: { requestType, requestId }, orderBy: { createdAt: "asc" } });
}
