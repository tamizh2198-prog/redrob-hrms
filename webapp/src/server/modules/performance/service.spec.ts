...
import * as performanceService from "./service";
...
describe("performance service", () => {
  ...
  describe("Review Cycle types: Monthly/Quarterly/Yearly", () => {
    beforeEach(() => {
      prisma.employee.findMany.mockResolvedValue([]);
      prisma.reviewCycle.create.mockImplementation(({ data }: { data: Record<string, unknown> }) =>
        Promise.resolve({ id: "cycle-new", ...data }),
      );
    });

    it("defaults to QUARTERLY when cycleType is omitted, preserving existing behavior", async () => {
      await performanceService.openReviewCycle(db, {
        name: "FY26 Q1",
    ...
    it("honors an explicit periodEnd as-is regardless of cycleType (no behavior change for existing callers)", async () => {
      ...
      expect(prisma.reviewCycle.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ periodEnd: new Date("2026-03-15") }) }),
      );
    });
    ...
    it("derives a 1-month period end for MONTHLY when periodEnd is omitted", async () => {
      ...
      expect(prisma.reviewCycle.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ cycleType: "MONTHLY", periodEnd: new Date("2026-02-01") }),
        }),
      );
    });
    ...
    it("derives a 3-month period end for QUARTERLY when periodEnd is omitted", async () => {
      ...
      expect(prisma.reviewCycle.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ cycleType: "QUARTERLY", periodEnd: new Date("2026-04-01") }),
        }),
      );
    });
    ...
    it("derives a 12-month period end for YEARLY when periodEnd is omitted", async () => {
      ...
      expect(prisma.reviewCycle.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ cycleType: "YEARLY", periodEnd: new Date("2027-01-01") }),
        }),
      );
    });
  });
  ...
  describe("Acceptance Criteria: goal weightage validation blocks submission if it does not sum to 100%", () => {
    it("rejects self-assessment submission when weightages sum to less than 100%", async () => {
      prisma.reviewCycle.findUnique.mockResolvedValue({ id: "cycle-1", status: "OPEN" });
      prisma.goal.findMany.mockResolvedValue([{ weightage: 40 }, { weightage: 30 }]);

      await expect(
        performanceService.submitSelfAssessment(db, { cycleId: "cycle-1", assessment: { notes: "x" } } as never, "emp-1"),
      ).rejects.toThrow("Goal weightages must sum to 100%");
    });

    it("allows submission once weightages sum to exactly 100%", async () => {
      prisma.reviewCycle.findUnique.mockResolvedValue({ id: "cycle-1", status: "OPEN" });
      prisma.goal.findMany.mockResolvedValue([{ weightage: 60 }, { weightage: 40 }]);
  ...
  describe("Acceptance Criteria: a review cannot close without both self and manager sections", () => {
    it("rejects closing the cycle when a review is missing a section", async () => {
      prisma.reviewCycle.findUnique.mockResolvedValue({ id: "cycle-1", status: "OPEN" });
      prisma.review.findMany.mockResolvedValue([{ id: "r1", selfAssessmentJson: { a: 1 }, managerAssessmentJson: null }]);

      await expect(performanceService.closeReviewCycle(db, "cycle-1", "hr-1")).rejects.toThrow(
        "missing a self or manager assessment",
      );
    });
    ...
    it("finalizes every review and closes the cycle once all sections are present", async () => {
      ...
      prisma.review.findMany.mockResolvedValue([
        { id: "r1", employeeId: "emp-1", selfAssessmentJson: { a: 1 }, managerAssessmentJson: { b: 1 } },
      ]);
      prisma.review.update.mockResolvedValue({});
      prisma.reviewCycle.update.mockResolvedValue({});

      const result = await performanceService.closeReviewCycle(db, "cycle-1", "hr-1");
  ...
  describe("Acceptance Criteria: a submitted rating is immutable outside the correction workflow", () => {
    it("rejects submitting a self-assessment against a closed cycle", async () => {
      prisma.reviewCycle.findUnique.mockResolvedValue({ id: "cycle-1", status: "CLOSED" });

      await expect(
        performanceService.submitSelfAssessment(db, { cycleId: "cycle-1", assessment: {} } as never, "emp-1"),
    ...
    it("rejects re-submitting a manager assessment once one already exists, even mid-cycle", async () => {
      ...
      await expect(
        performanceService.submitManagerAssessment(
          db,
          { cycleId: "cycle-1", employeeId: "emp-1", assessment: { rating: 5 }, rating: 5 } as never,
          "mgr-1",
        ),
      ).rejects.toThrow("already submitted for this review");
  ...
  describe("Access control: goals and manager assessments", () => {
    it("rejects an unrelated manager submitting an employee's manager assessment", async () => {
      prisma.employee.findUnique.mockResolvedValue({ reportingManagerId: "mgr-real" });

      await expect(
        performanceService.submitManagerAssessment(
          db,
          { cycleId: "cycle-1", employeeId: "emp-1", assessment: {}, rating: 4 } as never,
          "mgr-imposter",
          "MANAGER" as never,
        ),
      ).rejects.toThrow("Only this employee's manager or HR Admin");
    });

    it("rejects a manager creating a goal for someone who isn't their report", async () => {
      prisma.employee.findUnique.mockResolvedValue({ reportingManagerId: "someone-else" });

      await expect(
        performanceService.createGoal(
          db,
          { employeeId: "emp-2", cycleId: "cycle-1", title: "X", weightage: 50 } as never,
          "mgr-1",
          "MANAGER" as never,
        ),
      ).rejects.toThrow("Only the employee, their manager, or HR Admin");
    });
  ...
  describe("Acceptance Criteria: monthly KPI score maps to the correct grade", () => {
    it.each([
      ...
      prisma.monthlyEvaluation.upsert.mockImplementation(({ create }: { create: Record<string, unknown> }) =>
        Promise.resolve({ id: "eval-1", ...create }),
      );

      const result = await performanceService.submitMonthlyEvaluation(
        db,
        { employeeId: "emp-1", period: "2026-08-01", kpiScore, justification: "Solid month" } as never,
        "mgr-1",
      );
  ...
  describe("Acceptance Criteria: only the employee's assigned manager can submit a monthly evaluation", () => {
    it("rejects an unrelated manager submitting an employee's evaluation", async () => {
      ...
      await expect(
        performanceService.submitMonthlyEvaluation(
          db,
          { employeeId: "emp-1", period: "2026-08-01", kpiScore: 800, justification: "x" } as never,
          "mgr-imposter",
        ),
      ).rejects.toThrow("Only this employee's assigned manager");
    });
  ...
  describe("Acceptance Criteria: an approved evaluation cannot be resubmitted", () => {
    it("rejects resubmission once the evaluation is already APPROVED", async () => {
      prisma.employee.findUnique.mockResolvedValue({ id: "emp-1", reportingManagerId: "mgr-1" });
      prisma.monthlyEvaluation.findUnique.mockResolvedValue({ id: "eval-1", auditStatus: "APPROVED" });
      ...
      ).rejects.toThrow("already been audited and approved");
    });
  });
  ...
  describe("Acceptance Criteria: audit workflow can approve or send back a pending evaluation", () => {
    it("rejects auditing an evaluation that is not pending audit", async () => {
      ...
      await expect(
        performanceService.auditMonthlyEvaluation(db, "eval-1", { approve: true } as never, "hr-1"),
      ).rejects.toThrow("not pending audit");
    });

    it("requires auditNotes when sending an evaluation back", async () => {
      prisma.monthlyEvaluation.findUnique.mockResolvedValue({ id: "eval-1", auditStatus: "PENDING_AUDIT" });

      await expect(
        performanceService.auditMonthlyEvaluation(db, "eval-1", { approve: false } as never, "hr-1"),
      ).rejects.toThrow("auditNotes is required");
    });
    ...
    it("approves a pending evaluation and notifies the submitting manager", async () => {
      ...
      await performanceService.auditMonthlyEvaluation(db, "eval-1", { approve: true } as never, "hr-1");

      expect(notify).toHaveBeenCalledWith(
        db,
        expect.objectContaining({ recipientId: "mgr-1", template: "performance.monthly-evaluation-approved" }),
      );
    });
  });
  ...
  describe("Acceptance Criteria: employees see their score and grade, but never who scored them or why", () => {
    it("shows kpiScore and grade but redacts justification and submittedBy from the evaluation's own subject", async () => {
      ...
      const result = await performanceService.listMonthlyEvaluations(db, "emp-1", "emp-1", "EMPLOYEE" as never);

      expect(result[0].kpiScore).toBe(900);
      expect((result[0] as { grade: string }).grade).toBe("EE");
    ...
    it("shows the full record, including kpiScore, to the employee's manager", async () => {
      prisma.employee.findUnique.mockResolvedValue({ id: "emp-1", reportingManagerId: "mgr-1" });
      prisma.monthlyEvaluation.findMany.mockResolvedValue([{ id: "eval-1", employeeId: "emp-1", kpiScore: 900, grade: "EE" }]);

      const result = await performanceService.listMonthlyEvaluations(db, "emp-1", "mgr-1", "MANAGER" as never);
    ...
    it("rejects an unrelated employee viewing someone else's evaluations", async () => {
      ...
      await expect(
        performanceService.listMonthlyEvaluations(db, "emp-1", "emp-2", "EMPLOYEE" as never),
    ...
    it("rejects an HR Admin who isn't this employee's manager — only Super Admin gets the company-wide bypass", async () => {
      ...
      await expect(
        performanceService.listMonthlyEvaluations(db, "emp-1", "hr-1", "HR_ADMIN" as never),
      ).rejects.toThrow("Not authorized to view this employee's evaluations");
    });
    ...
    it("lets a Super Admin view any employee's evaluations", async () => {
      ...
      const result = await performanceService.listMonthlyEvaluations(db, "emp-1", "sa-1", "SUPER_ADMIN" as never);
  ...
  describe('submitMonthlyEvaluation notifies real Super Admins (fixes the dead "hr-admin" placeholder)', () => {
    it('notifies every Super Admin, not the literal string "hr-admin"', async () => {
      ...
      const { notify } = jest.requireMock("../../lib/notify");
      await performanceService.submitMonthlyEvaluation(
        db,
        { employeeId: "emp-1", period: "2026-08-01", kpiScore: 800, justification: "x" } as never,
        "mgr-1",
      );

      expect(notify).toHaveBeenCalledWith(db, expect.objectContaining({ recipientId: "sa-1" }));
      expect(notify).toHaveBeenCalledWith(db, expect.objectContaining({ recipientId: "sa-2" }));
  ...
  describe("Monthly score release gating: hidden from the subject until approved AND past the 3rd of next month", () => {
    it("hides the score from the subject while still pending audit", async () => {
      ...
      expect((result[0] as { grade: string | null }).grade).toBeNull();
      expect((result[0] as { auditStatus: string }).auditStatus).toBe("PENDING_AUDIT");
    });

    it("hides an APPROVED score from the subject until the 3rd of the month after the period", async () => {
      prisma.monthlyEvaluation.findMany.mockResolvedValue([
        // Today is well after 2026-08 per the test clock, but this period is
        // deliberately the *current* month so its release date (3rd of next
        // month) hasn't arrived yet regardless of when this suite runs
        // relative to the 3rd.
      ...
      expect(result[0].kpiScore).toBeNull();
      expect((result[0] as { releaseDate: Date }).releaseDate).toEqual(new Date(Date.UTC(9999, 1, 3)));
    });
  ...
  describe("Release-notification cron queries", () => {
    it("findDueMonthlyReleases returns only APPROVED, not-yet-notified rows past their release date", async () => {
      ...
      const due = await performanceService.findDueMonthlyReleases(db);

      expect(due.map((e: { id: string }) => e.id)).toEqual(["eval-past"]);
      expect(prisma.monthlyEvaluation.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { auditStatus: "APPROVED", releaseNotifiedAt: null } }),
      );
    });

    it("markMonthlyReleaseNotified stamps releaseNotifiedAt", async () => {
      await performanceService.markMonthlyReleaseNotified(db, "eval-1");
