...
// dependencies" — no csv/xlsx/pdf library exists anywhere in this codebase
// beyond exceljs, so all three formats below are hand-built rather than
...
// Produces a genuine .xlsx via exceljs (already a real dependency, used by
// the Employee bulk-import template and active-employees export).
...
export function toPdf(result: ReportExportInput): Buffer {
  ...
  objects.push(`<< /Type /Pages /Kids [${pageIds.map((id) => `${id} 0 R`).join(" ")}] /Count ${pages.length} >>`); // id 2
...
export async function exportReport(result: ReportExportInput, format: ReportExportFormat): Promise<{ buffer: Buffer; contentType: string; extension: string }> {
  switch (format) {
    case "csv":
      return { buffer: Buffer.from(toCsv(result), "utf-8"), contentType: "text/csv", extension: "csv" };
    case "excel":
      return {
    ...
    case "pdf":
      return { buffer: toPdf(result), contentType: "application/pdf", extension: "pdf" };
  }
}
