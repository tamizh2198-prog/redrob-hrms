...
// Bulk-upload counterpart to the single-employee "Assign WFO Days" form:
// one row per employee, one column per weekday.
export async function parseHybridScheduleWorkbook(buffer: Buffer): Promise<BulkHybridScheduleRow[]> {
  ...
  const weekdayCols = WEEKDAY_COLUMNS.map(({ header, day }) => ({ day, col: columnIndex.get(header.toLowerCase()) }));

  const rows: BulkHybridScheduleRow[] = [];
  sheet.eachRow((row, rowNumber) => {
    if (rowNumber === 1) return;
    const employeeCode = employeeCodeCol ? String(row.getCell(employeeCodeCol).value ?? "").trim() : "";
...
export async function buildHybridScheduleTemplate(): Promise<Buffer> {
  ...
  sheet.columns = [
    ...
    ...WEEKDAY_COLUMNS.map(({ header }) => ({ header, key: header, width: 6 })),
  ];
  sheet.addRow({
