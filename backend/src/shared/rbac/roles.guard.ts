...
// Fixed, role-based default module access: unlike the per-employee
// ModuleAccessGrant below (a Super Admin-managed exception on one
// employee), an entry here would give every holder of that role automatic
// operational access to the listed modules and nothing else, without
// acting as a general alias for a higher-privileged role like HR_ADMIN or
// SUPER_ADMIN on other modules. Exported so the Roles & Permissions UI
// (permissions.service.ts) can display a role's real, enforced access
// instead of maintaining a second, independent list that could drift out
...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (requiredModule) {
      // Role-based default — checked first since it's a synchronous,
      // no-DB-lookup case; falls through to the per-employee grant lookup
