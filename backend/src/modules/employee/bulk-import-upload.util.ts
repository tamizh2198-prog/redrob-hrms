...
// Department/Designation/Grade/Location/Reporting Manager only accept the
// internal record id (not a name) — same constraint the existing JSON-paste
// import already has. Left blank in the example row; HR can look these ids
// up from the Employee Directory / org-lookup screens.
...
// and immutable (see EmployeeService.generateEmployeeCode), never accepted
// from an upload.
export async function parseEmployeeImportWorkbook(
  buffer: Buffer,
): Promise<Partial<CreateEmployeeDto>[]> {
  const workbook = new ExcelJS.Workbook();
  // Same nominal-Buffer<T> cast as parseHybridScheduleWorkbook — exceljs's
  // bundled type defs predate current @types/node.
